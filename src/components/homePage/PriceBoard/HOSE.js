...
function HOSE() {
  const [hose_instruments, setDataHose] = useState([]);
  const {search}= useSelector((state) => state.search)

  useEffect(() => {
    axios
      .get(`http://localhost:3000/d`, {
        params: {
          _limit: 20,
          symbol_like: search,
        },
  ...
  // let get20Data = hose_instruments.slice(0, 20);
  const [data, setData] = useState(hose_instruments);
  ...
  const ChangeData = () => {
    ...
    hose_instruments.map((info, index) => {
      if (
        info.bidPrice1 &&
        info.bidPrice2 &&
        info.bidPrice3 &&
        ...
        return (
          (info.bidPrice3_classHighLight = getHighLight(
            info.bidPrice3,
            bidPrice3,
            info
          )),
          (info.bidPrice2_classHighLight = getHighLight(
            info.bidPrice2,
            bidPrice2,
            info
          )),
          (info.bidPrice1_classHighLight = getHighLight(
            info.bidPrice1,
            bidPrice1,
            info
          )),
          (info.offerPrice1_classHighLight = getHighLight(
            info.offerPrice1,
            offerPrice1,
            info
          )),
          (info.offerPrice2_classHighLight = getHighLight(
            info.offerPrice2,
            offerPrice2,
            info
          )),
          (info.offerPrice3_classHighLight = getHighLight(
            info.offerPrice3,
            offerPrice3,
            info
          )),
          (info.closePrice_classHighLight = getHighLight(
            info.closePrice,
            closePrice,
            info
          )),
  ...
  const tableHOSE = hose_instruments.map((info) => {
    return (
      <>
        <tr>
          <td
            className={setColor("symbol", info, info.closePrice)}
            onClick={() => hanldeClick(info)}
          >
            {info.symbol}
          </td>
          ...
          <td className={setColor("bidPrice3", info, info.bidPrice3)}>
            {Calculate(info.bidPrice3)}
          </td>
          <td className={setColor("bidVol3", info, info.bidPrice3)}>
            {Calculate(info.bidVol3)}
          </td>
          <td className={setColor("bidPrice2", info, info.bidPrice2)}>
            {Calculate(info.bidPrice2)}
          </td>
          <td className={setColor("bidVol2", info, info.bidPrice2)}>
            {Calculate(info.bidVol2)}
          </td>
          <td className={setColor("bidPrice1", info, Number(info.bidPrice1))}>
            {Calculate(Number(info.bidPrice1))}
          </td>
          <td className={setColor("bidVol1", info, Number(info.bidPrice1))}>
            {Calculate(info.bidVol1)}
          </td>

          <td className={setColor("closePrice", info, info.closePrice)}>
            {Calculate(info.closePrice)}
          </td>
          <td className={setColor("closeVol", info, info.closePrice)}>
            {Calculate(info.closeVol)}
          </td>
          <td className={setColor("change", info, info.closePrice)}>
            {info.change > 0 ? "+" : ""}
            {Calculate(info.change)}
          </td>
          <td className={setColor("change", info, info.closePrice)}>
            {info.change > 0 ? "+" : ""}
            {Calculate(info.change)}
          ...
          <td
            className={setColor("offerPrice1", info, Number(info.offerPrice1))}
          >
            {Calculate(Number(info.offerPrice1))}
          </td>
          <td className={setColor("offerVol1", info, Number(info.offerPrice1))}>
            {Calculate(info.offerVol1)}
          </td>
          <td className={setColor("offerPrice2", info, info.offerPrice2)}>
            {Calculate(info.offerPrice2)}
          </td>
          <td className={setColor("offerVol2", info, info.offerPrice2)}>
            {Calculate(info.offerVol2)}
          </td>
          <td className={setColor("offerPrice3", info, info.offerPrice3)}>
            {Calculate(info.offerPrice3)}
          </td>
          <td className={setColor("offerVol3", info, info.offerPrice3)}>
            {Calculate(info.offerVol3)}
          </td>

          <td className="TVAL">{Calculate(info.totalTrading)}</td>
          <td className="TVOL">{Calculate(info.totalTradingValue)}</td>

          <td className={setColor("high", info, info.high)}>
            {Calculate(info.high)}
          </td>
          <td className={setColor("averagePrice", info, info.averagePrice)}>
            {Calculate(info.averagePrice)}
          </td>
          <td className={setColor("low", info, info.low)}>
            {Calculate(info.low)}
          </td>
          <td></td>
          <td></td>
  ...
  return (
    <div className="modal display-block">
      <section className="modal-main">
        <div className="name-popup">{details?.FullName}</div>
        ...
        <button onClick={handleClose} id="btn-close">
          Close
        </button>
      </section>
    </div>
